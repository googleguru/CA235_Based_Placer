"""
Ultra-fast comparison demo with synthetic benchmark for quick visualization output.
"""
...
def make_bigblue_overlay(data_obj: PlacementData, density_map, ex_field, ey_field):
    ...
    bin_width = (data_obj.die_xh - data_obj.die_xl) / w
    bin_height = (data_obj.die_yh - data_obj.die_yl) / h
    
    for x, y in movable_pos:
        gx = int((x - data_obj.die_xl) / bin_width)
        gy = int((y - data_obj.die_yl) / bin_height)
...
def run_fast_comparison():
    """Run all 3 algorithms on smaller SYNTHETIC benchmark for speed."""
    
    # Use synthetic data (5K cells) for FAST demo
    print("\n" + "="*80)
    print("DREAMPlace MetaOpt - ALGORITHM COMPARISON (Synthetic: 5K cells)")
    print("="*80)
    
    data = load_or_generate_benchmark(num_cells=5000, num_bins=32, seed=42)
    print(data.summary())
    
    algorithms = [
        ("SBO",    HybridSBO,           {"max_iterations": 20,   "num_clusters": 30}),
        ("PSO",    PSOWithSBO,          {"max_iterations": 100,  "num_clusters": 30, "num_particles": 15}),
        ("CA235",  CellularAutomataCA235, {"max_iterations": 30}),
    ]
    ...
    fig.suptitle("DREAMPlace MetaOpt - Algorithm Comparison (Synthetic: 5K cells)", 
    ...
    outpath = os.path.join("results", "COMPARISON_TABLE_all_algorithms.png")
